...
const csrf = (options = {}) => {
    ...
    return async (c, next) => {
        ...
        const origin = c.req.header("Origin") || config_1.config.corsOrigins[0];
        // For all requests, set CORS headers
        c.res.headers.set("Access-Control-Allow-Origin", origin);
        c.res.headers.set("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS");
        c.res.headers.set("Access-Control-Allow-Headers", "Content-Type, X-CSRF-Token");
        ...
        if (!headerToken || headerToken !== cookieToken) {
            console.log(`CSRF Middleware - Invalid token: ${headerToken} vs ${cookieToken}`);
