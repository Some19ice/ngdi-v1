...
async function main() {
  ...
  try {
    ...
    const ngdiMetadataCount = await prisma.nGDIMetadata.count()
    ...
    const ngdiRecords = await prisma.nGDIMetadata.findMany({
      include: {
    ...
    console.log(
      `Retrieved ${ngdiRecords.length} records from NGDIMetadata table`
    )
    ...
    for (const record of ngdiRecords) {
      try {
        ...
        if (existingMetadata) {
          console.log(
            `Skipping record with title "${record.dataName}" - already exists in Metadata table`
          )
          skippedCount++
          continue
        }
        ...
        const metadataRecord = {
          ...
          dateFrom:
            record.productionDate || new Date().toISOString().split("T")[0],
          dateTo:
            record.productionDate || new Date().toISOString().split("T")[0],
